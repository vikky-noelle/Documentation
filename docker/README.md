...
- Every Image has a default `ENTRYPOINT` and `CMD` that it uses in case a container is based off it.
- Default **ENTRYPOINT** in ubuntu based systems are `/bin/sh -c` while the **CMD** is `bash`.
...
- `docker create [OPTIONS] IMAGE [COMMAND] [ARG...]`
  It creates a new container from an image.

  You might wonder about the difference between `docker run` and `docker create`, `docker run = docker create + docker start + docker attach`.
